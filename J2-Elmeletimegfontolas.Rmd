...
```{r theme_economist, echo=FALSE, message=FALSE, warning=FALSE}
library(ggplot2)
library(ggthemes)
theme_set(
...
```{r p1_baseindexes, fig.height=7, fig.width=10, message=FALSE, warning=FALSE}
library(tidyverse)
...
ggplot(df, aes(x = Year, y = value)) +
  ...
  labs(
    title = "1. ábra. Születési mutatók bázisindexe", subtitle = "1960=100%",
    caption = "A bázisindex használatát a hárommutató együttesen való ábrázolásának céljából tartottam indokoltnak."
  ) +
  xlab("Év") +
  ylab("Százalék") +
...
```{r p2_fertility_replacementrate, fig.height=7, fig.width=10}
...
ggplot(df, aes(x = Year, y = value)) +
  ...
  scale_x_continuous(expand = c(0, 0)) +
  labs(
    title = "2. ábra. Magyarország termékenységi rátájának alakulása", subtitle = "1960-2018",
    caption = "Az ábra alapján jól látható, hogy az érték a vizsgált időszakban jelentősen a 2,1-es egyszerű reprodukció határát jelentő érték alatt maradt, globális minimumát (1,23) pedig 2011-ben érte el."
  ) +
...
df <- data.frame(
  c(
    ...
    "jövedelem és foglalkoztatás mutatói",
    "megtakarítások és eladósodás mutatói",
    "bűnözési statisztikák, császármetszések aránya",
    "nyugdíjkifizetések mértéke, szegénységi mutatók"
  )
)
names(df) <- c(
  "Szerző", "Szerző által mely országra fogalmazódott meg a hipotézis",
  "Forrás alapján elemzésembe fókuszába helyezett indikátorok"
)
df %>%
  kable(
    caption = "Szakirodalom áttekintése alapján dolgozatom témájának releváns gazdasági- és társadalmi indikátorainak köre", align = c("c", "c"), booktaps = F
  ) %>%
  kable_styling(
